...
export default function PushNotifications() {
 ...
 const enable = async () => { try { setState('loading'); if (!(await isSupported())) throw new Error(); const permission=await Notification.requestPermission(); if(permission!=='granted') {setState('denied');return;} const app=getApps().length?getApp():initializeApp(config); const registration=await navigator.serviceWorker.register('/firebase-messaging-sw.js'); const token=await getToken(getMessaging(app),{vapidKey:import.meta.env.VITE_FIREBASE_VAPID_KEY,serviceWorkerRegistration:registration}); if(!token) throw new Error(); router.post(route('push-devices.store'),{token},{preserveScroll:true,onSuccess:()=>setState('enabled'),onError:()=>setState('error')}); } catch { setState('error'); } };
