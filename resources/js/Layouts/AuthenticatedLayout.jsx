        ...
        <div className="mx-auto min-h-[100dvh] max-w-6xl bg-[#f6f8f7] pb-24 shadow-sm sm:pb-8">
            <nav className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/90 px-5 py-3 backdrop-blur">
                <div className="mx-auto flex max-w-5xl items-center justify-between">
                    ...
                    <div className="flex items-center gap-2"><Link href={route('notifications.index')} className="relative hidden size-10 items-center justify-center rounded-full bg-slate-50 text-slate-500 sm:flex" aria-label="Notifications"><Bell size={19}/>{unreadCount > 0 && <span className="absolute -right-1 -top-1 flex min-w-5 justify-center rounded-full bg-rose-500 px-1 text-[10px] font-bold leading-5 text-white">{unreadCount > 99 ? '99+' : unreadCount}</span>}</Link><Link href={route('profile.edit')} className="flex size-10 items-center justify-center rounded-full bg-[#e0f8ea] font-bold text-[#13a856]" title="Profile">{user.name.slice(0, 1).toUpperCase()}</Link></div>
                </div>
            </nav>
            ...
            <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 px-3 pb-[max(.65rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur sm:hidden"><div className="mx-auto flex max-w-md items-center justify-around">{nav.map(([name, label, Icon]) => <Link key={name} href={route(name)} className={`flex min-w-16 flex-col items-center gap-1 text-[10px] font-bold uppercase tracking-wider ${current === name || (name === 'trackers.index' && current.startsWith('trackers.')) ? 'text-[#18c968]' : 'text-[#94a3b8]'}`}><span className="relative"><Icon size={21} strokeWidth={2.4}/>{name === 'notifications.index' && unreadCount > 0 && <span className="absolute -right-3 -top-2 flex min-w-5 justify-center rounded-full bg-rose-500 px-1 text-[10px] font-bold leading-5 text-white">{unreadCount > 99 ? '99+' : unreadCount}</span>}</span>{label}</Link>)}</div></nav>
        </div>
    </div>;
}
